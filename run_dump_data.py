...
if __name__ == "__main__":
    ...
    movies["year"] = (
        movies["title"].str.extract(r"\((\d{4})\)").fillna("1995").astype(int)
    )

    popularity = (
        interactions.groupby("movieId").agg({"userId": "nunique"}).reset_index()
    )
    ...
    movies = movies.merge(popularity, on="movieId", how="left").drop(
        columns="number_of_users_interacted",
    )
